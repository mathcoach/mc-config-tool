...
/**
 * An API for translating text, taking from Apache Common Text.
...
abstract class CharSequenceTranslator {
    ...
    final String translate(final CharSequence input) {
        ...
        try {
            ...
            throw new RuntimeException(ioe);//NOSONAR (obviously Sonar should not check this line of code)
        }
    }
    ...
    final void translate(final CharSequence input, final Writer out) throws IOException {        
        Objects.requireNonNull(out, "The Writer must not be null");
        if (input == null) {
            return;
        }
        int pos = 0;
